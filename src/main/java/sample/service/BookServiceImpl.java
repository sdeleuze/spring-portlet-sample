...
public class BookServiceImpl extends ApplicationObjectSupport
	implements BookService {

	private final SortedMap<Integer, Book> books = Collections
			.synchronizedSortedMap(new TreeMap<Integer, Book>());

	private byte[] loadPngFile(String location) {
		try {
			return IOUtils.toByteArray(getApplicationContext().getResource(
					location).getInputStream());
		} catch (IOException e) {
			throw new BeanInitializationException("unable to load png file '"
					+ location + "'", e);
		}
	}
	...
	private URL toUrl(String website) {
		try {
			...
			throw new BeanInitializationException("unable to create URL '"
					+ website + "'", e);
		}
	}
